  ...
  const pagePolicies={
    trusted:{
      ...
      trader:{
        ...
        'use-applications':pagePolicy('申请办理','创建数据使用申请，维护草稿并跟踪各数据提供方的审批进度。',['新建申请','编辑草稿','提交撤回','审批进度']),
    ...
    intermodal:{
      ...
      trader:{
        'my-demands':pagePolicy('需求办理','创建并提交运输需求，申请联运协同数据后进入方案生成。',['新建需求','编辑提交','申请数据','撤回需求']),
        'solution-center':pagePolicy('方案决策','使用已授权的联运能力数据生成、比较并确认联运方案。',['方案生成','方案比选','数据来源','确认方案']),
