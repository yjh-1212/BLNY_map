(function(){
  ...
  function flowHtml(){const w=state.workflow;if(!w)return'';const approved=(w.requests||[]).find(x=>x.status==='已通过')?.count||0;const done=(w.tasks||[]).filter(x=>x.status==='已完成').length;const steps=[['运输需求',w.demand?.status||'未创建',!!w.demand],['四方申请',`${approved}/4 已授权`,approved===4],['联运方案',w.scheme?.status||'未生成',!!w.scheme],['运输订单',w.order?.status||'未生成',!!w.order],['分段任务',`${done}/${w.tasks?.length||4} 已完成`,done===4],['审计闭环',w.order?.status==='已完成'?'已形成':'进行中',w.order?.status==='已完成']];let active=steps.findIndex(x=>!x[2]);if(active<0)active=steps.length-1;return `<section class="biz-flow"><div class="biz-flow-head"><b>玉米5,000吨 · 绥化粮库—广东目的港业务闭环</b><span>所有状态均来自SQLite实时数据</span></div><div class="biz-flow-steps">${steps.map((x,i)=>`<div class="biz-flow-step ${x[2]?'done':i===active?'active':''}"><small>0${i+1}</small><b>${esc(x[0])}</b><small>${esc(x[1])}</small></div>`).join('')}</div></section>`}
  function actionButtons(row){
    ...
    if(state.resource==='demands'&&r==='trader'){
      ...
      if(row.status==='已提交')out.push(`<button class="biz-btn small primary" data-biz="apply-data" data-id="${row.id}">申请四方数据</button>`);
