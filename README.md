...
系统采用前后端分离架构：后端使用 Express 5 提供 JSON API，并使用 SQLite 本地文件保存业务数据；前端使用 Vue 3、Vue Router 和 Vite。各业务页面与共享资源保存在 `frontend/public/`，Vue 负责统一路由和跨模块导航。
...
数据库默认创建在 `backend/data/beiliang-nanyun.db`。数据库初始化是幂等的，重复执行不会重复插入初始化业务数据。
...
系统提供六类业务角色：平台管理员、贸易商业务负责人、公路货运调度员、铁路调度员、港口协同员和船商调度员。顶部切换角色后，可信数据空间与多式联运会同步加载该角色可见的菜单、企业数据范围和操作权限。
...
角色化 CRUD API 位于 `/api/v1/business/:resource`，流程 API 位于 `/api/v1/workflow/*`。前端会自动携带当前角色，并由后端按企业、订单参与关系和分段任务进行二次权限校验。完整业务流程如下：
...
- Vue 路由为每个业务模块提供稳定地址，并通过 `postMessage` 支持跨模块导航。
- 业务页面使用同源全屏框架隔离，避免 Vue 全局样式影响业务页面视觉。
- 共享导航统一处理角色、企业数据范围、菜单权限和页面操作策略。
